...
A native profile has one Foundry process owner at a time to avoid multiple refresh writers. Foundry refuses native-profile auxiliary sessions. Current production startup hardcodes a separate OpenAI/Luna API decision provider. This does not satisfy the subscription-only rollout: the Codex adapter currently refuses text-only decision sessions, so an enforced native decision path is still required. Do not provision a paid API key or silently substitute another model to bypass that gap. An external CLI does not honor Foundry's lock; coordinate external use of the same profile as well.
