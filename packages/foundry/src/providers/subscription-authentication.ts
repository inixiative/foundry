...
export class SubscriptionAuthentication extends NativeAuthentication {
  ...
  constructor(directory: string, private workerSource: Extract<NativeAuthenticationSource, { mode: "native-profile" }>, private statusSpawn?: (profile: string) => StatusProcess, private statusDeadlineMs = STATUS_DEADLINE_MS) {
  ...
  override async prepare(threadId: string, runtime: "claude" | "codex"): Promise<NativeAuthenticationLaunch> {
    ...
    if (this.statusClosed) throw Error("Subscription status admission unavailable; no API fallback");
    if (Date.now() - this.statusVerifiedAt > STATUS_TTL_MS) {
      this.statusCheck ??= this.verifyStatus().finally(() => { this.statusCheck = undefined; });
      try { await this.statusCheck; } catch { throw Error("Subscription worker unavailable; no API fallback"); }
    }
  ...
  private async verifyStatus(): Promise<void> {
    ...
    try {
      const status = await Promise.race([
        ...
        new Promise<never>((_, reject) => { timer = setTimeout(() => reject(Error("Subscription status deadline")), this.statusDeadlineMs); }),
      ]);
