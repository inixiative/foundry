import type { JobHandler, JobHandlerArgs } from "./types";

const REDIS_NS = "foundry:job";
...
export const makeSingletonJob = <TPayload = void>(handler: JobHandler<TPayload>): JobHandler<TPayload> => {
  return async (ctx, ...args: JobHandlerArgs<TPayload>) => {
    ...
    const lockKey = `${REDIS_NS}:lock:${jobData.id}`;
    const lockTTL = 300;

    const acquired = await redis.set(lockKey, "1", "EX", lockTTL, "NX");
    if (!acquired) return;

    const heartbeat = setInterval(() => redis.expire(lockKey, lockTTL).catch(() => {}), 120_000);
