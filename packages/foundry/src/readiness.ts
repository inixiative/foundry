...
export async function inspectReadiness(config: FoundryConfig, options: {
  ...
  const checkProvider = (id: string, scope: string) => {
    ...
    if (id !== config.defaults.provider && !(id === "openai" && config.providers.openai?.enabled))
  ...
  for (const source of config.nativeAuthentication ?? []) {
    ...
    if (source.mode === "native-profile") {
      ...
      if (!(config.providers.openai?.enabled && environment.OPENAI_API_KEY)) issue("error", source.id, "native-profile-decision-provider", "The native profile has one refresh owner. Production startup requires the separate OpenAI decision provider.");
