...
|---------|----------|-------|-------|--------|-------------------|-----------|
| Email/password + OAuth | Yes | Yes | Yes | Yes | Yes | Usually |
| Per-org OAuth/SAML config | Yes (self-managed, AES-256-GCM encrypted) | Enterprise add-on | Yes (vendor-managed) | Yes (vendor-managed) | No | Rare |
| Bearer tokens (SHA-256 hashed, Redis-cached) | Yes | No | No | No | Basic plugin | Usually plaintext |
| URL token fallback | Yes | No | No | No | No | No |
| Hierarchical token scopes (User/Org/Space) | Yes | No | No | No | No | No |
| PATs with usage tracking | Yes | Limited | No | No | No | Rare |
| Superadmin spoofing with audit headers | Yes | Actor JWT | Deprecated | Yes (dashboard only) | No | Direct DB access |
| Per-token rate limiting | Yes | No | No | No | No | No |
...
|---------|----------|---------------|---------|---------|-----------|--------|------|-----------|
| RBAC + ABAC + ReBAC unified | Yes | Yes | Yes | Yes | Yes | RBAC + ABAC | RBAC + ABAC | Usually flat RBAC |
...
| Auto-route enforcement middleware | Yes | No | No | No | No | Via sidecar | No | Custom |
...
|---------|----------|--------------------|---------|---------|-----------|----------|
| Transaction-aware hooks (execute inside tx, roll back with it) | **Yes** | No | No hooks | Hooks fire before commit | Hooks fire before commit | Hooks fire before commit |
| Post-commit callbacks | **Yes** (`db.onCommit()`) | No | No | Community pkg | **Native** (`afterCommit`) | No |
| Auto transaction merging (ALS + Proxy) | **Yes** | No (open request) | Community lib | Community lib | ALS propagation (no merge) | Community |
| Branded ID types | **Yes** (18 types) | No (plain string) | No | No | No | No |
| Test factories with relationship inference | **Yes** | No | No | No | No | No |
| Hydration with N+1 prevention | **Yes** (PendingMap) | `include` (no dedup) | No | Eager loading | Eager loading | `populate` (partial) |
...
| Frontend stack | React 19 + Vite + TanStack (ecosystem consensus choices) |
| DX tooling | Turborepo + Biome + Bun (fast, modern, minimal config) |
